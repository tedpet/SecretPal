...
public class SPProfilePage extends SPPage {
	private static final Logger LOG = LoggerFactory.getLogger(SPProfilePage.class);
...
//			}
			...
			if (!Objects.equals(SPPerson.hashPassword(_currentPassword), _person.password())) {
			    _currentPassword = null;
			    session().errors().addNotice("Your current password does not match the password you provided.");
			    return null;
			}
...
//			}
			...
			if (!Objects.equals(_password, _confirmPassword)) {
			    _password = null;
			    _confirmPassword = null;
			    session().errors().addNotice("Your password confirmation didn't match.");
			    return null;
