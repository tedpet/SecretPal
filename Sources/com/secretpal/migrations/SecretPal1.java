...
/**
 * I found a bunch of email addresses where people invited with
 * "<mschrag@whatever.com>". This migration goes and corrects those email
 * addresses, which are no longer allowed.
 * 
 * @author mschrag
 */
public class SecretPal1 extends ERXMigrationDatabase.Migration implements IERXPostMigration {
	...
	public void postUpgrade(EOEditingContext editingContext, EOModel model) throws Throwable {
		...
		NSArray<SPPerson> thePersons = SPPerson.fetchAllSPPersons(editingContext);
		for (SPPerson person : thePersons) {
			...
			try {
				...
				// this probably means the person realized they screwed up and reinvited the
				// person with the proper email address
